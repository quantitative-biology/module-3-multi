...
```{r perkclust, echo=TRUE, comments="", fig.cap="Cluster figure of isotope data from Perkins et al. 2014)", fig.alt="This figure shows a cluster diagram of isotope data from Perkins et al. with y-axis labelled 'Height' that scales from 0 to 5. "}
str(iso)
...
```{r coriris, echo=TRUE, comment="", fig.cap="correlation atrix for the iris data", fig.alt="The graph shows a total of twelve pairwise correlation plots given four dimensions in Iris dataset – the length of sepal, the width of sepal, the length of petal, and the width of petal. In each pairwise correlation plot, one of the four dimensions is selected as x axis and one is selected as y aixs. Data points are colored in red, green and blue based on its species."}
...
``` {r shep, echo=FALSE, fig.caption="Shepard's plot of an nMDS ordination using 2 axes to represent the varespec data in the vegan package", fig.alt="This figure shows two plots placed side by side. The left plot has x-axis labelled 'Observed Dissimilarity' that scales from 0.0 to 0.4 and a y-axis with label 'Ordination Distance' that scales from 0.0 to 0.8. The left plot shows a cluster of blue points that form a thickened line that trends upwards. The top of the plot also has text with label 'Non-metric fit, R^2^ = 0.999' and 'Linear fit, R^2^ = 0.995'. The right plot has x-axis labelled 'NMDS1' that scales from -0.4 to 0.4 and y-axis labelled 'NMDS2' that scales from -0.6 to 0.6. The plot shows two clusters of points beside each other near the center of the plot. Scattered between the clusters are four red cross symbols. From left to right, they are labeled 'Sepal.Width', 'Sepal.Length', 'Petal.Length' and 'Petal.Width'."}
library(vegan)
data(varespec)
...
```{r, fig.alt="This figure shows two plots placed side by side. The left plot is titled 'PCoA', has x-axis labelled 'PCoA1' that scales from -0.6 to 0.6 and y-axis labelled 'PCoA2' that scales from -0.6 to 0.6. The plot contains 24 scattered points which represent the sites and 44 scattered red variables which represent the species. The right plot is tilted 'nMDS', has x-axis labelled 'MDS1' that scales from -0.6 to 0.6 and y-axis labelled 'MDS2' that scales from -0.6 to 0.6. The plot contains 24 scaterred points which represent the sites as well as 44 scattered red variables which represent the species."}
par(mfrow=c(1,2))
