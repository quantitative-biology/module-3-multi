...
```{r perkclust, echo=TRUE, comment="", fig.cap="Cluster diagram of isotope data from Perkins et al. 2014)", fig.alt="Y-axis is labelled 'Height' and x-axis is labelled 'diso'."}
str(iso)
...
```{r clustdata, echo=TRUE, fig.cap="Data from Perkins et al (2014) with grouping from single linkage clustering superimposed, where calculated clusters 1 to 4 and species categories with varying symbols", fig.alt="This figure has x-axis labelled 'δ13C', y-axis labelled 'δ15N'."}
...
```{r varord, fig.cap="Selecting the synthetic axes in ordination. We have two overlaid plots where those axes labelled x are the original data, and those labelled y are our synthetic axis. The two guassian functions suggest how much variation is present on each the the two y synthetic axes.  "}
...
```{r scree, echo=TRUE, fig.cap="Screeplot: a display of the variance explained by each of the synthetic axes. The variances decrease for each subsequent axis", fig.alt="The graph shows the relationship between variances and the number of axes that explain the dataset. As the number of axes increases from 1 to 4, the variances decrease from 3 to 0.",comment=""}
