...
```{r clust, echo=TRUE, fig.cap="Single linkage clustering on Perkins et al (2014) data with outlier removed", fig.alt="This figure shows a dendrogram with single linkage clustering with y-axis labelled 'Height' that scales from 0 to 5."}
...
It doesn't look like our cluster algorithm is matching up with our Food.chain data categories very well. Wheat- and Nettle-based food chains cannot be distinguished, which makes sense when you consider that both of these plants use a C3 photosynthesis system. If you are not happy with the success of this clustering algorithm you could try other variants (e.g.,  “complete” linkage) and a different number of groups. 
...
* The adjustment goes on until the difference between the observed and modelled dissimiarity matrices (called "stress"), can cannot be lowered any further, or until it reaches a predetermined low value (tolerated lack-of-fit). 
...
### Exercise: Ordination
We are going to use the vegan package, and some built in data with it to run the nMDS. Varespec is a data frame of observations of 44 species of lichen at 24 sites. We’ll calculate both an NMDS and a PCoA using the (cmdscale() function) on the bray-curtis distance matrix of these data. In each case, we will specify that we want 2 dimensions as our output. The vegan wrapper for nMDS ordination (**metaMDS()** standardizes our community data by default, using a Wisconsin transform. This is a method of double standardization that avoids negative values in the transformed data, and is completed by first standardizing species data using the maxima, and then the site by totals. We will have to apply this standardization manually for the PCoA analysis.
...
If we look at the object PCoA we see eigenvalues, one for each of the 20 sites, and the new 2D coordinates for each site. We can plot the results as plot(PCoA$points). But, since this will be a crowded plot, let's use a vegan package function **ordipointlabel()** which will use an optmization routine to select the best position for all the labels. We'll plot the PCoA and the NMDS side by side to see if they differ, using the **par(mfrow())** functions. In this case, our species are the variables and our sites are the objects of our attention. There's a lot of species, so we won't draw the arrows given by the eigenvectors, we'll just show the of the end of the vector on the biplot. 
...
text(x=-15, y=31, labels = "Variables y1 ... yp",cex = 1.)
text(x=-15, y=17.5, labels = "matrix Y", cex = 1.2, font = 2)
text(x=-21, y=17.5, labels = "Objects 1 to n", cex = 1.2, srt=90)
text(x=-15, y=34, labels = "Simple ordination: PCA/CA",cex = 1.1)
segments(5,5,10,5)
segments(5,30,5,5)
segments(5,30,10,30)
segments(10,5,10,30)
text(x=7.5, y=31, labels = "Variable y",cex = 1.)
text(x=7.5, y=17.5, labels = "y", cex = 1.2, font = 2)
text(x=4, y=17.5, labels = "Objects 1 to n", cex = 1., srt=90)
segments(15,5,30,5)
segments(15,30,15,5)
segments(30,5,30,30)
segments(15,30,30,30)
text(x=22.5, y=31, labels = "Variables x1 ... xm",cex = 1.)
text(x=22.5, y=17.5, labels = "matrix X", cex = 1.2, font = 2)
text(x=17, y=34, labels = "Multiple regression",cex = 1.1)

arrows(10.5,17.5,14.5,17.5,length=0.05, lwd=2)
arrows(14.5,17.5,10.5,17.5,length=0.05, lwd=2)
text(x=0, y=-3, labels = "Ordination under constraint: RDA/CCA",cex = 1.1)
segments(-10,-10,-2,-10)
segments(-10,-35,-10,-10)
segments(-2,-35,-2,-10)
segments(-10,-35,-2,-35)
text(x=-5, y=-8, labels = "Variables y1 ... yp",cex = 1.)
text(x=-6, y=-22.5, labels = "matrix Y", cex = 1.2, font = 2)
text(x=-11, y=-22.5, labels = "Objects 1 to n", cex = 1., srt=90)
segments(2,-10,2,-35)
segments(2,-10,14,-10)
segments(2,-35,14,-35)
segments(14,-10,14,-35)
text(x=8, y=-8, labels = "Variables x1 ... xm",cex = 1)
text(x=8, y=-22.5, labels = "matrix X", cex = 1.2, font = 2)
arrows(-1.5,-22.5,1.5,-22.5,length=0.05, lwd=2)
arrows(1.5,-22.5,-1.5,-22.5,length=0.05,lwd=2)
```
...
We will use a forumla interface, to constrain the ordination. The shortcut ., indicates that we want to use all the variables in dune.env The output shows the eigenvalues for both the constrained and unconstrained ordination axes. In particular, see how the inertia and rank (number of axes) are decomposed in constrained ordination.
...
Examine the output. How did we do? Unsurprisingly, we have not explained as much variance as the full model. In addition, the amount of variance (inertia) explained by the constrained axes is less than the unconstrained axes. Maybe we shouldn't be using this analysis at all. Of course, some of the variables in the full model may or may not be important. We need a signficance test to try and sort this out.

### Signficance tests for constrained ordination

The vegan package provides permutation tests for the significance of constraints. The test mimics standard analysis of variance function, and the default test analyses all constraints simultaneously.
...
The analysis suggests that maybe N is not explaining a signficant amount of variation. This test is sequential: the terms are analyzed in the order they happen to be in the model, so you may get different results depending on how the model is specified. See if you can detect this, by rearranging your model terms in the call to ordination. 

NB It is also possible to analyze the significances of marginal effects (“Type III effects") using anova(ord, by="mar", permutations=199). Moreover, it is possible to analyse signicance of each axis: anova(ord, by="axis", permutations=499)

### Forward Selection of explanatory variables
You may wonder if you have selected the correct variables! We can use the **ordiR2step()** function to perform forward selection on a null model.  In automatic model building we usually need two extreme models: the smallest and the largest model considered. We need to build a model with all environmental variables and a model with no environmental variables, but both with a formula so that terms can be added or removed from the model. We will then use ordiR2step()  to find an optimal model based on both permutation and adjusted R2 values.
...
Examine the significance test. Does this model differ from the full model and your manually selected version? 

```{r}
optm
```

Next check the variance (i.e., inertia). If the constrained variance is not much higher than your unconstrained variance, then analysis suggests that the rda is not really needed.
...
Some VIF values are above 10 or even 20 in the full model, so that a reduction of the number of explanatory variables is justified. The VIF values in the reduced model are quite lower.

Finally, let’s examine the adjusted $R^2$ of the model

```{r, echo=TRUE, comment=""}

paste("full model:",round(RsquareAdj(mfull)$adj.r.squared,2))
paste("reduced model:",round(RsquareAdj(optm)$adj.r.squared,2))
```
...
Plotting an ordination is not as simple as it sounds, as I mentioned above, one can choose various scaling conventions that will dramatically change the appearance and interpretation of the plot.  One of the editors of the vegan package, Gavin Simpson, has more to say about this (here)[https://fromthebottomoftheheap.net/2015/10/08/user-friendly-scaling/]. Essentially, there can be no scaling ("none"), or either the data object ("site") or characterisitc ("species") scores are scaled by eigenvalues, and the other set of scores is left unscaled, or both scores are scaled symmetrically by square root of eigenvalues ("symmetric").  I've plotted the first three types below. Were you able to produce the same ordinations?

```{r triplots, echo=TRUE, comment="", fig.cap="Triplot of an RDA ordination on the lichen dataa with scaling to preserve angles of the species vectors", fig.alt="This figure shows a plot with y-axis labelled 'RDA2' and x-axis labelled 'RDA1'. The plot contains the variable names of the 44 species in the varaspec dataset scattered throughout the plot. There are three blue arrows that emerge from the point (0,0). One arrow points towards the top left of the plot and has label 'P'. Another arrow points towards the right and has label 'Fe'. The last arrow points slightly towards the bottom left of the plot and has label 'Mn'. There are 22 numbers scaterred near the center of the plot."}
# Triplots of the parsimonious RDA (scaling=1)
# Scaling 1
plot(optm, scaling=1)
```
```{r triplotscale2, echo=TRUE, comment="",fig.cap="Triplot of an RDA ordination on the lichen dataa with no scaling ", fig.alt="This figure shows a plot with y-axis labelled 'RDA2' and x-axis labelled 'RDA1'. The plot contains the variable names of the 44 species in the varaspec dataset clumped at the center of the plot. There are three blue arrows that emerge from the point (0,0). One arrow points towards the top left of the plot and has label 'P'. Another arrow points towards the right and has label 'Fe'. The last arrow points slightly towards the bottom left of the plot and has label 'Mn'. There are 22 numbers scaterred throughout the plot."}
# Triplots of the parsimonious RDA (scaling=2)
# Scaling 2
plot(optm, scaling="species")
```

```{r triplotscale0, echo=TRUE, comment="", fig.cap="Triplot of an RDA ordination on the lichen dataa with no scaling ", fig.alt="This figure shows a plot with y-axis labelled 'RDA2' and x-axis labelled 'RDA1'. The plot contains the variable names of the 44 species in the varaspec dataset grouped near the center of the plot. There are three blue arrows that emerge from the point (0,0). One arrow points towards the top left of the plot and has label 'P'. Another arrow points towards the right and has label 'Fe'. The last arrow points slightly towards the bottom left of the plot and has label 'Mn'. There are 22 numbers scaterred throughout the plot."}
# Triplots of the parsimonious RDA (scaling=0)
# Scaling 0
plot(optm, scaling=0)
```
